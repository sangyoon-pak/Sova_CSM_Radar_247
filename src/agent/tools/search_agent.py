...
RERANK_PROMPT = """You score how relevant each document snippet is to the user's question.
...
If most snippets are irrelevant (wrong product, wrong topic), set sufficient to false.
Output ONLY the JSON object."""

REFINE_PROMPT = """The initial search did not find enough relevant content. Refine the search terms.

Original question: {query}
Reason insufficient: {reason}

Output a JSON object with refined search terms:
{{ "terms": ["phrase1", "phrase2", "product name", ...] }}
...
- Output 3-5 terms. Output ONLY the JSON object."""
...
def _rerank_matches(query: str, matches: list[dict], threshold: int = 3) -> list[dict]:
    ...
    for m in matches:
        line = m.get("line", "")[:200]
        snippets.append(f"[{m['file']} L{m['line_num']}] {line}")
...
def _check_sufficient(query: str, matches: list[dict]) -> tuple[bool, str]:
    ...
    llm = _get_llm()
    preview = "\n".join(m.get("line", "")[:150] for m in matches[:10])
...
def _refine_search_terms(query: str, reason: str) -> list[str]:
    """LLM produces refined search terms."""
    llm = _get_llm()
...
def search_with_agent(
    ...
    for iteration in range(max_iterations):
        if iteration == 0:
            ...
            if sufficient:
                break
            terms = _refine_search_terms(query, reason)
            if not terms:
                break
