...
- **UI:** **Run history** (per-run feedback: OK / NG / Good / Noise — each submit stores a short **note** so the row is eligible for distillation) and **Action dashboard** (card-level text + optional `action_index`; see [ACTION_CARD_SPEC.md](ACTION_CARD_SPEC.md)). **Configure → Distilled learning rules** shows the current **`agent_learning_instructions`** text (same payload as `GET /memory/learning` and the `distilled_learning` field on `GET /settings/runtime`). Distillation reads only feedback rows with non-empty **note** or **correction** (`get_learning_feedback_samples`).
